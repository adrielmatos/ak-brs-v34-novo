...
def ler_xlsx_sem_openpyxl(file_bytes):
    # Tenta com openpyxl primeiro - mais confiavel
    try:
        import openpyxl
        try:
            return pd.read_excel(BytesIO(file_bytes), engine='openpyxl')
        except Exception as e:
            # Se openpyxl existe mas falhou, tenta sem engine
            try:
                return pd.read_excel(BytesIO(file_bytes))
            except:
                raise e
    except ImportError:
        # openpyxl nao instalado - tenta fallback manual e avisa
        pass
    
    # Fallback manual para quando openpyxl nao esta no requirements.txt
    try:
        ...
        try:
            ...
            pass
        raise ValueError("openpyxl nao instalado no requirements.txt - fallback manual falhou. Adicione 'openpyxl' no Requirements.txt")
    except zipfile.BadZipFile:
        raise ValueError("XLSX invalido ou corrompido")

def ler_planilha(up):
    nome=up.name.lower()
    # Le bytes uma vez e reusa
    try:
        ...
        try:
            ...
            file_bytes=up.read() if hasattr(up, 'read') else b''
    ...
    if nome.endswith(".xlsx"):
        try:
            ...
            if "openpyxl" in str(e).lower():
                raise ValueError(f"{up.name}: {e} | SOLUCAO: adicione 'openpyxl' no Requirements.txt")
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"{up.name}: precisa de 'xlrd' no Requirements.txt - {e2}")
        except Exception as e:
            try:
                return pd.read_excel(BytesIO(file_bytes))
            except:
    ...
    # CSV com varios encodings e separadores
    for enc in ["utf-8","latin1","cp1252","iso-8859-1"]:
        for sep in [",",";","\t","|"]:
            try:
                df=pd.read_csv(BytesIO(file_bytes), encoding=enc, sep=sep, engine='python')
                if len(df.columns)>1:
                    return df
            except:
                continue
    raise ValueError(f"{up.name}: Nao consegui ler - formato nao reconhecido")
        ...
        if arquivos:
            ...
            if erros:
                for err in erros:
                    st.error(err)
                if not dfs:
                    st.error("❌ Nenhum dos arquivos enviados pôde ser lido. Verifique as dependências no requirements.txt - precisa de 'openpyxl' para XLSX e 'xlrd' para XLS")
                    st.info("💡 Solucao: adicione no Requirements.txt:\nopenpyxl\nxlrd\n\nDepois de salvar, clique em Reboot no Streamlit Cloud")
            if dfs:
