...
if not st.session_state.leads:
    ...
    custo_total = df_all['custo_estimado'].sum() if 'custo_estimado' in df_all.columns else tempo_total/60*0.15
    tmo = tempo_total / max(atend+nao_at+vendas,1)
    tma = df_all[df_all['status']!='pendente']['duracao_seg'].mean() if len(df_all[df_all['status']!='pendente'])>0 else 0
    cpc = (atend+vendas)/max(atend+nao_at+vendas,1)*100 if (atend+nao_at+vendas)>0 else 0
    total_pausas_seg = sum([p['duracao_seg'] for p in st.session_state.pausas])
    
    c1,c2,c3,c4,c5,c6,c7,c8 = st.columns(8)
    c1.metric("📥 Fila", pend)
    c2.metric("⏱️ TMO", formatar_tempo(tmo))
    c3.metric("📞 TMA", formatar_tempo(tma))
    c4.metric("🎯 CPC", f"{cpc:.0f}%")
    c5.metric("💰 Custo Chip", f"R$ {custo_total:.2f}")
    c6.metric("⏸️ Pausas", formatar_tempo(total_pausas_seg))
    c7.metric("💰 Vendas", vendas)
    c8.metric("📱 Chip", "Online" if not em_pausa else "Pausa")

    col_rank1, col_rank2 = st.columns(2)
    with col_rank1:
        st.markdown("##### 🏆 Ranking Bancos CHIP")
        ranking_lig = df_all[df_all['status']!='pendente'].groupby('banco').agg({'id':'count','duracao_seg':'sum','custo_estimado':'sum'}).rename(columns={'id':'LIGAÇÕES','duracao_seg':'TEMPO_SEG','custo_estimado':'CUSTO'}).reset_index()
        ranking_lig['TEMPO']=ranking_lig['TEMPO_SEG'].apply(formatar_tempo)
        ranking_lig['CUSTO']=ranking_lig['CUSTO'].apply(lambda x: f"R$ {x:.2f}")
        ranking_lig = ranking_lig.sort_values('LIGAÇÕES', ascending=False)
        st.dataframe(ranking_lig[['banco','LIGAÇÕES','TEMPO','CUSTO']].head(10), use_container_width=True, hide_index=True)
        
        ranking_vendas = df_all[df_all['status']=='venda_finalizada'].groupby('banco').size().reset_index(name='VENDAS')
        ranking_completo = pd.merge(ranking_lig, ranking_vendas, on='banco', how='outer').fillna(0)
        ranking_completo['CONVERSÃO']= (ranking_completo['VENDAS']/ranking_completo['LIGAÇÕES']*100).round(1)
        st.dataframe(ranking_completo[['banco','LIGAÇÕES','VENDAS','CONVERSÃO']].sort_values('VENDAS', ascending=False).head(10), use_container_width=True, hide_index=True)
        if len(ranking_completo)>0:
            st.bar_chart(ranking_completo.set_index('banco')['LIGAÇÕES'].head(8))

    with col_rank2:
        st.markdown("##### ⏸️ Pausas")
        if st.session_state.pausas:
            df_pausas = pd.DataFrame(st.session_state.pausas)
            st.dataframe(df_pausas.tail(10), use_container_width=True, hide_index=True)
        else:
            st.info("Nenhuma pausa")
        
        st.markdown("##### 🤖 Tabulação")
        tab_counts = df_all['tabulacao'].value_counts().reset_index().head(8)
        tab_counts.columns=['TABULAÇÃO','QTD']
        st.dataframe(tab_counts, use_container_width=True, hide_index=True)

    if st.button("📊 GERAR EXCEL CHIP SAFE - 12 ABAS - Sem Preditivo", type="primary", use_container_width=True):
        output = BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df_all.to_excel(writer, sheet_name='GERAL_CHIP_SAFE', index=False)
            df_all[df_all['status']=='pendente'].to_excel(writer, sheet_name='FILA_URA', index=False)
            df_all[df_all['status']=='venda_finalizada'].to_excel(writer, sheet_name='VENDAS_CHIP', index=False)
            ranking_lig.to_excel(writer, sheet_name='RANKING_CHIP', index=False)
            ranking_completo.to_excel(writer, sheet_name='CONVERSAO_CHIP', index=False)
            timeline=[]
            for lead in st.session_state.leads:
                for h in lead.get('historico',[]):
                    timeline.append({"nome":lead['nome'],"banco":lead['banco'],"telefone":lead['telefone'],"data":h.get('data'),"acao":h.get('acao'),"tempo":h.get('tempo'),"custo":h.get('custo'),"tab":h.get('tabulacao')})
            pd.DataFrame(timeline).to_excel(writer, sheet_name='TIMELINE_CHIP', index=False)
            pd.DataFrame(st.session_state.pausas).to_excel(writer, sheet_name='PAUSAS', index=False)
            tab_counts.to_excel(writer, sheet_name='TABULACAO', index=False)
        st.download_button("⬇️ BAIXAR EXCEL CHIP SAFE", output.getvalue(), file_name=f"BRS_CHIP_SAFE_SEM_PREDITIVO_{datetime.now().strftime('%d%m%Y_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True, type="primary")
        st.balloons()
        st.success("✅ Chip Safe: Power + Preview apenas, 1 ligação por vez, sem risco de 2 atenderem")
