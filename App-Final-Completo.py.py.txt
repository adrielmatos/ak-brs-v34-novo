...
def proximo_inteligente(atual_id=None):
    # FILA INTELIGENTE - prioriza quem nunca foi ligado, depois retorno, depois tentativa menor
    pend = [l for l in st.session_state.leads if l['status']=='pendente']
    if not pend:
        return None
    # Ordena: tentativas 0 primeiro, depois por ultima (Nunca primeiro), depois por banco que mais vende
    vendas_por_banco = {}
    ...
    def score(l):
        ...
        # banco que mais vende ganha -2 pontos
...
# CSS MINI DASHBOARD FLUTUANTE - sem poluir
st.markdown("""
<style>
.mini-dash {
  position: fixed;
...
# HEADER LIMPO v5.2
st.markdown("## 📱 A&K BRS v5.2 CLEAN PRO | Leve sem avatares")
...
with col_h1:
    ...
    st.caption(f"📱 {total} | 📥 {pend} pendentes | 🧠 Fila Inteligente | ⚡ 1 clique | 🎯 Modo Foco | Leve")
with col_h2:
...
# MODO FOCO - se em ligação, esconde lista
...
if not st.session_state.leads:
    ...
    for l in st.session_state.leads:
        ...
        if 'busca' in locals() and busca and busca.lower() not in l['nome'].lower() and busca.lower() not in l['banco'].lower(): continue
        lista.append(l)
    
    if modo_foco_ativo:
        # MODO FOCO - só atendimento gigante
        col_atend = st.container()
        with col_atend:
            ...
            if sel:
                ...
                # Tabulação 1 clique gigante
                st.markdown("#### ⚡ Tabulação 1 clique (sem digitar)")
                ...
                def finalizar_foco(status_final, obs_pronta):
                    ...
                    st.rerun()
                with c1:
                    if st.button("✅ Atendeu - interessado", use_container_width=True, type="primary", key=f"foco_at1_{sel['id']}"): finalizar_foco('atendido','Atendeu - interessado, explicar FGTS')
                with c2:
                    if st.button("🔴 Caixa postal", use_container_width=True, key=f"foco_cx_{sel['id']}"): finalizar_foco('nao_atendeu','Caixa postal')
                with c3:
                    if st.button("📵 Desligado", use_container_width=True, key=f"foco_des_{sel['id']}"): finalizar_foco('nao_atendeu','Desligado / fora de área')
                with c4:
                    if st.button("💰 Venda!", use_container_width=True, key=f"foco_vd_{sel['id']}"): st.balloons(); finalizar_foco('venda_finalizada','Venda FGTS - enviar docs')
                c5,c6,c7,c8=st.columns(4)
                with c5:
                    if st.button("🤔 Sem interesse", use_container_width=True, key=f"foco_sem_{sel['id']}"): finalizar_foco('atendido','Sem interesse no momento')
                with c6:
                    if st.button("📅 Retorno amanhã 14h", use_container_width=True, key=f"foco_ret_{sel['id']}"): finalizar_foco('retorno_futuro','Retornar amanhã 14h')
                with c7:
                    if st.button("❌ Número errado", use_container_width=True, key=f"foco_err_{sel['id']}"): finalizar_foco('nao_atendeu','Número errado / não existe')
                with c8:
        ...
        with col_lista:
            ...
            st.caption("🧠 Ordenado por: nunca ligados > menor tentativa > banco que mais vende")
            for lead in lista[:80]:
                ...
                tempo_txt = f" {lead.get('duracao_txt','')}" if lead.get('duracao_seg',0)>0 else ""
                tent_txt = f" T{lead.get('tentativas',0)}"
                if st.button(f"{'👉' if is_sel else ''}{dot} {lead['nome'][:14]} • {lead['banco']}{tent_txt}{tempo_txt}", key=f"list_{lead['id']}", use_container_width=True, type="primary" if is_sel else "secondary"):
        ...
        with col_atend:
            if not st.session_state.selected_id:
                ...
                if sel:
                    ...
                    st.caption(f"Tentativas: {sel['tentativas']} | Última: {sel['ultima']} | Status: {sel['status']} | Custo: R$ {sel.get('custo_estimado',0):.2f}")
                    numero=''.join(filter(str.isdigit, sel['telefone']))
                    ...
                    if em_pausa:
                        ...
                        decorrido = (datetime.now()-st.session_state.call_start[sel['id']]).total_seconds()
                        st.warning(f"📱 EM LIGAÇÃO: {formatar_tempo(decorrido)} - Modo Foco ativo")
                    ...
                    st.markdown("#### ⚡ Tabulação 1 clique (economiza digitação)")
                    c1,c2,c3,c4=st.columns(4)
                    ...
                    with c8:
                        obs_custom = st.text_input("Obs livre", key=f"obs_{sel['id']}", placeholder="livre", label_visibility="collapsed")
                        if st.button("💾 Salvar obs", use_container_width=True, key=f"fin_obs_{sel['id']}") and obs_custom:
                            finalizar('atendido',obs_custom)

# MINI DASHBOARD FLUTUANTE
if st.session_state.leads:
    df_all = pd.DataFrame(st.session_state.leads)
    pend = len(df_all[df_all['status']=='pendente'])
    atend = len(df_all[df_all['status']=='atendido'])
    nao_at = len(df_all[df_all['status']=='nao_atendeu'])
    vendas = len(df_all[df_all['status']=='venda_finalizada'])
    tempo_total = df_all['duracao_seg'].sum()
    tmo = tempo_total / max(atend+nao_at+vendas,1)
    cpc = (atend+vendas)/max(atend+nao_at+vendas,1)*100 if (atend+nao_at+vendas)>0 else 0
    total_pausas = sum([p['duracao_seg'] for p in st.session_state.pausas]) if st.session_state.pausas else 0
    st.markdown(f"""
    <div class="mini-dash">
      📥 {pend} | ⏱️ TMO {formatar_tempo(tmo)} | 🎯 CPC {cpc:.0f}% | 💰 {vendas} vendas | ⏸️ {formatar_tempo(total_pausas)}
    </div>
    """, unsafe_allow_html=True)
...
with col_exp1:
    with st.expander("📊 Kanban Visual", expanded=False):
        if st.session_state.leads:
            leads_filtrados = [l for l in st.session_state.leads if st.session_state.filtro_banco=="TODOS" or l['banco']==st.session_state.filtro_banco]
            kanban_cols = st.columns(5)
            statuses = [("pendente","📥 Fila"), ("atendido","✅ Atendidos"), ("nao_atendeu","❌ Caixa"), ("retorno_futuro","🟠 Retornos"), ("venda_finalizada","💰 Vendas")]
            for idx, (status_key, status_label) in enumerate(statuses):
                with kanban_cols[idx]:
                    ...
                    for lead in lista_status[:8]:
                        if st.button(f"{lead['nome'][:12]} • {lead['banco']}", key=f"kanban_{lead['id']}_{status_key}_{idx}", use_container_width=True):
                            st.session_state.selected_id=lead['id']; st.session_state.modo_foco=False; st.rerun()
with col_exp2:
    with st.expander("📈 Dashboard Completo", expanded=False):
        if st.session_state.leads:
            df_all = pd.DataFrame(st.session_state.leads)
            c1,c2,c3,c4 = st.columns(4)
            total = len(df_all)
            ...
            tempo_total = df_all['duracao_seg'].sum()
            c1.metric("📥 Fila", pend)
            c2.metric("⏱️ Falado", formatar_tempo(tempo_total))
            c3.metric("💰 Vendas", vendas)
            c4.metric("📊 Total", total)
            ranking = df_all[df_all['status']=='venda_finalizada'].groupby('banco').size().reset_index(name='VENDAS')
            if not ranking.empty:
                st.dataframe(ranking.sort_values('VENDAS', ascending=False), use_container_width=True, hide_index=True)
with col_exp3:
    with st.expander("📥 Exportar Excel", expanded=False):
        if st.session_state.leads and st.button("📊 Gerar Excel", type="primary", use_container_width=True):
            output = BytesIO()
            df_all = pd.DataFrame(st.session_state.leads)
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                df_all.to_excel(writer, sheet_name='GERAL', index=False)
            st.download_button("⬇️ Baixar", output.getvalue(), file_name=f"BRS_v5_2_CLEAN_PRO_{datetime.now().strftime('%d%m%Y_%H%M')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
